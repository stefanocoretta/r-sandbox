...
scrape_this <- "https://www.journal-labphon.org/articles/?page="
base_URL <- "https://www.journal-labphon.org"

links <- c()
for (i in 1:9) {
  page <- read_html(paste0(scrape_this, i))
  links <- c(links, html_attr(html_nodes(page, "a"), "href"))
}

# minor string cleanup
articles <- links[str_detect(links, "article/")] %>%
  str_squish()

# construct URLs
articles_URLS <- paste0(base_URL, articles)
...
for (i in articles_URLS) {
  page <- read_html(i)
  down_links <- c(links, html_attr(html_nodes(page, "a"), "href"))
}

downloads <- down_links[str_detect(down_links, "download/")] %>%
  str_squish()

download_URLS <- paste0(base_URL, downloads)
